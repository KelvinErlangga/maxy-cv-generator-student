...
@section('title', 'Link Informasi | CVRE GENERATE')
...
<div class="min-h-screen flex flex-col relative bg-gradient-to-b from-white via-purple-50 to-blue-50">
    ...
    <div class="absolute top-10 left-40 z-10">
        <a href="{{ route('cv', ['section' => 'hasil_prestasi']) }}" class="text-blue-700 hover:underline text-sm flex items-center">
    ...
    <!-- Form Container -->
    <form id="linkForm" method="POST" action="{{ route('cv', ['section' => 'keterampilan_teknis']) }}" class="bg-white shadow-lg rounded-lg p-8 mx-auto z-10 mb-20" style="max-width: 800px; width: 100%;">
        @csrf
                ...
                <div class="flex items-center space-x-4 w-full">
                    ...
                    <div class="grid grid-cols-2 gap-4 w-full">
                        <div class="col-span-1">
                            <input type="text" name="link[]" class="link-input block w-full rounded border border-gray-300 focus:ring-blue-500 focus:border-blue-500 focus:ring-2 focus:outline-none" style="height: 45px; padding: 0 10px;" placeholder="Masukkan Link Informasi" required />
                        </div>
                        <div class="col-span-1">
                            <input type="text" name="level[]" class="desc-input block w-full rounded border border-gray-300 focus:ring-blue-500 focus:border-blue-500 focus:ring-2 focus:outline-none" style="height: 45px; padding: 0 10px;" placeholder="Masukkan Deskripsi" required />
                        </div>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('linkForm').addEventListener('submit', function (event) {
            ...
            for (let i = 0; i < links.length; i++) {
                if (links[i].value.trim() === '' || descriptions[i].value.trim() === '') {
                    alert('Silakan isi semua field "Masukkan Link Informasi" dan "Masukkan Deskripsi" sebelum melanjutkan!');
